...
try{
    ...
    if (isset($_GET['success1']) && $_GET['success1'] == true) {
        echo '<script>alert("Chỉnh sửa lớp thành công!");</script>';
    }
?>
...
<tbody>
  ...
  foreach ($result as $rs){
    ...
    $queryGiangVien = new MongoDB\Driver\Query(['ma_giang_vien' => $maGiangVien]);
    $resultGiangVien = $conn->executeQuery('QLSV.Giangvien', $queryGiangVien);

    // Lấy bản ghi đầu tiên từ kết quả (nếu có)
    $giangVien = current($resultGiangVien->toArray());
...
<td class="text-right">
<div class="actions">
<a href="edit-class.php?ma_lop='.$rs -> {'ma_lop'}.'" class="btn btn-sm bg-success-light mr-2">
