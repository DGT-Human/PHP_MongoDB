...
try{
    ...
    if (isset($_GET['success1']) && $_GET['success1'] == true) {
        echo '<script>alert("Chỉnh sửa sinh viên thành công!");</script>';
    }
                              ...
                              <table class="table table-hover table-center mb-0 datatable">
                                 <thead>
                                  ...
                                  foreach ($result as $rs){
                                      echo '<tr>';
                                      echo ('<td>'.$rs -> {'ma_sinh_vien'}.'</td>');
                                      echo  '<td>';
                                          echo  '<h2 class="table-avatar">';
                                            echo '<a href="student-details.html">'.$rs -> {'ten'}.'</a>';
                                          echo '</h2>';
                                       echo  '</td>';
                                      echo ('<td>'.$rs -> {'ma_lop'}.'</td>');
                                      ...
                                      echo '<a href="edit-student.php?ma_sinh_vien='.$rs -> {'ma_sinh_vien'}.'" class="btn btn-sm bg-success-light mr-2">';
                                      echo '<i class="fas fa-pen"></i>';
                                      echo '</a>';
                                      echo '<a href="#" class="btn btn-sm bg-success-light mr-2" onclick="updateStatus(\'' . $rs->{'ma_sinh_vien'} . '\');">';
                                      echo '<i class="fa-solid fa-lock"></i>';
                                      echo '</a>';
                                      echo '<a href="students-subject_ID.php?ma_sinh_vien='.$rs -> {'ma_sinh_vien'}.'" class="btn btn-sm bg-success-light mr-2">';
                                      echo '<i class="fa-solid fa-book"></i>';
                                      echo '</a>';
                                      echo '<a href="student-detail1.php?ma_sinh_vien='.$rs -> {'ma_sinh_vien'}.'" class="btn btn-sm bg-success-light mr-2">';
                                      echo '<i class="fa-solid fa-address-card"></i>';
                                      echo '</a>';
                                      echo ' </div>';
                                      echo ' </td>';
                                      echo ' </tr>';
                                  ...
                                  function updateStatus(studentId) {
                                    ...
                                    xhr.open("GET", "update-status.php?ma_sinh_vien=" + studentId, true);
      ...
      <script>
          ...
          function searchStudent() {
              ...
              for (i = 0; i < tr.length; i++) {
                  ...
                  var classColumn = tr[i].getElementsByTagName("td")[1];
                  var dobColumn = tr[i].getElementsByTagName("td")[2];
                  var sexColumn = tr[i].getElementsByTagName("td")[3];
                  var mobileColumn = tr[i].getElementsByTagName("td")[4];
                  var statusColumn = tr[i].getElementsByTagName("td")[5];

                  if (nameColumn || classColumn || dobColumn || sexColumn || mobileColumn || statusColumn) {
                      var nameText = nameColumn.textContent || nameColumn.innerText;
                      var classText = classColumn.textContent || classColumn.innerText;
                      var dobText = dobColumn.textContent || dobColumn.innerText;
                      var sexText = sexColumn.textContent || sexColumn.innerText;
                      ...
                      if (nameText.toUpperCase().indexOf(filter) > -1 ||
                          classText.toUpperCase().indexOf(filter) > -1 ||
                          dobText.toUpperCase().indexOf(filter) > -1 ||
                          sexText.toUpperCase().indexOf(filter) > -1 ||
