...
$query = new MongoDB\Driver\Query(['id_DK' => $ma_dang_ky],[]);
...
<ul>
    ...
    <li class="submenu active">
        ...
        <ul>
            ...
            <li><a href="students-subject.php" class="active">Subject Sign List</a></li>
        </ul>
    </li>
    <li class="submenu ">
        ...
        <div class="content container-fluid">
            ...
            <div class="page-header">
                <div class="row align-items-center">
                    <div class="col">
                        <h3 class="page-title">Edit Subject Sign</h3>
                        <ul class="breadcrumb">
...
<div class="card">
<div class="card-body">
<form action="update-student-subject-action.php" method="post">
<div class="row">
<div class="col-12">
...
<label>ID_dangky</label>
    <?php
        foreach ($result as $rs) {
            echo '<label class="form-control" id="searchInput">'.$rs->{'id_DK'}.'</label>';
            echo '<input type="hidden" class="form-control" name="id_DK" value="' . $rs->{'id_DK'} . '">';
            echo '</div>
            </div>';
            ...
            echo '<label class="form-control" id="searchInput">'.$rs->{'ma_sinh_vien'}.'</label>';
            echo '</div>';
            echo '</div>';
            echo '<div class="col-12 col-sm-6">';
            echo '<div class="form-group">';
...
//                $query1 = new MongoDB\Driver\Query(['ma_hoc_phan' => $rs1->{'ma_hoc_phan'}], []);
                ...
                foreach ($result2 as $rs2) {
                    $query2 = new MongoDB\Driver\Query(['ma_hoc_phan' => $rs2->{'ma_hoc_phan'}], []);
                    $result3 = $conn->executeQuery('QLSV.Hocphan', $query2);
                    foreach ($result3 as $rs3) {
                        echo '<option value="' . $rs2->{'ma_nhom_hoc_phan'} . '" ' . (($rs->{'ma_nhom_hoc_phan'} == $rs2->{'ma_nhom_hoc_phan'}) ? 'selected' : '') . '>' . $rs3->{'ten_hoc_phan'} .': '.$rs2->{'ma_nhom_hoc_phan'}. '</option>';
                    }
                }
...
<script>
    $(document).ready(function () {
        // Tạo một ô input cho việc tìm kiếm
        $('#searchInput').on('input', function () {
            var searchTerm = $(this).val().toLowerCase();

            // Lặp qua từng option trong select và ẩn/hiện dựa trên từ khóa tìm kiếm
            $('select[name="id_sub"] option').each(function () {
                var textToSearch = $(this).text().toLowerCase();
                var isMatch = textToSearch.includes(searchTerm);
                $(this).toggle(isMatch);
            });
        });
    });
